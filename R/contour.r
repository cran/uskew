...
fmmst.contour.2d <- function(dat, model, grid=50, drawpoints=TRUE, clusters=NULL,
    ...
    if(map[1]=="heat") {    
        ...
        plot(x = 0, y = 0,type = "n", xlim = rx, ylim = ry,xlab=xlab,ylab=ylab, main=msg)        
        for(i in 1:g) points(X[clustergrid==i,], pch=22, lwd=grid/5, col=ColClust[i], bg=ColClust[i])
    }else plot(x = 0, y = 0,type = "n", xlim = rx, ylim = ry, xlab=xlab, ylab=ylab, main=msg)
...
fmmst.contour.3d <- function(dat, model, grid=20, drawpoints=TRUE, levels=0.9, 
    ...
    if(drawpoints) {
        plot3d(dat[clusters==1,1], dat[clusters==1,2], dat[clusters==1,3], 
            col="magenta", xlab=xlab, ylab=ylab, zlab=zlab, xlim=rx, ylim=ry, zlim=rz)
        if(g>=2){ 
            for(i in 2:g)            
            points3d(dat[clusters==i,1], dat[clusters==i,2], dat[clusters==i,3], col=ColPoint[i])        
        }
    } else {
        plot3d(1,1,1,col="white", xlab=xlab, ylab=ylab, zlab=zlab, xlim=rx, ylim=ry, zlim=rz)        
    } 
    XVec <- seq(rx[1], rx[2], length=grid)       
    YVec <- seq(ry[1], ry[2], length=grid)
    ZVec <- seq(rz[1], rz[2], length=grid) 
    if(is.null(component)) {
...
contour3d <- function(f, level, x = 1:dim(f)[1], y = 1:dim(f)[2], z = 1:dim(f)[3],
    ...
    col.mesh = if (fill) NA else color, material = "default", smooth = 0,
